...
;
(function(){function a(){typeof Swal<"u"&&Swal.fire({position:"top-end",icon:"success",title:"Email Sent!",showConfirmButton:!1,timer:900})}function s(e){var t="Something went wrong!";e&&(e.text||e.message)&&(t=e.text||e.message),e&&e.status===412&&!e.text&&!e.message&&(t="EmailJS rejected request (412). Free plan cannot use domain whitelist. Check Email History and reconnect Email Service."),typeof Swal<"u"?Swal.fire({icon:"error",title:"Oops...",text:t}):alert(t)}function i(){var e=document.getElementById("contact-form");if(!(!e||typeof emailjs>"u")&&e.dataset.emailBound!=="true"){e.dataset.emailBound="true";try{emailjs.init("BDr8Qs-jUxeKirf96")}catch{}e.addEventListener("submit",function(t){t.preventDefault();var n=e.querySelector('button[type="submit"]');n&&(n.disabled=!0);var m={name:(e.elements.name&&e.elements.name.value||"").trim(),email:(e.elements.email&&e.elements.email.value||"").trim(),company:(e.elements.company&&e.elements.company.value||"").trim(),service:(e.elements.service&&e.elements.service.value||"").trim(),message:(e.elements.message&&e.elements.message.value||"").trim()};emailjs.send("service_kda5tug","template_1ip2c0j",m,"BDr8Qs-jUxeKirf96").then(function(){a(),e.reset()}).catch(function(l){s(l)}).finally(function(){n&&(n.disabled=!1)})})}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",i):i()})();
